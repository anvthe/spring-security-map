...
INSERT INTO doctors (name, doctor_bmdc)
...
SELECT l.latitude, l.longitude, COUNT(*) as count
FROM prescriptions p
         JOIN prescription_brands pb ON p.id = pb.prescription_id
         JOIN brands b ON pb.brand_id = b.id
         JOIN locations l ON p.location_id = l.id
WHERE b.id = ?1
  AND l.id = ?2 GROUP BY l.latitude, l.longitude;



SELECT b.brand_name, l.location_name, l.latitude, l.longitude
FROM prescriptions p
         JOIN prescription_brands pb ON p.id = pb.prescription_id
         JOIN brands b ON pb.brand_id = b.id
         JOIN locations l ON p.location_id = l.id
WHERE b.id = ?1
  AND l.id = ?2 GROUP BY b.brand_name, l.location_name, l.latitude, l.longitude

@Query(value = "SELECT b.brand_name, l.location_name, l.latitude, l.longitude, COUNT(*) as count FROM prescriptions p JOIN prescription_brands pb ON p.id = pb.prescription_id JOIN brands b ON pb.brand_id = b.id JOIN locations l ON p.location_id = l.id WHERE b.id = ?1 AND l.id = ?2 GROUP BY b.brand_name, l.location_name, l.latitude, l.longitude", nativeQuery = true)